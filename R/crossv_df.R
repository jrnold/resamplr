...
#' @param train A list of integer vectors, with the indices of the training set.
#'   If \code{train} is \code{NULL}, then the training set is the complement of
#'   the test set.
#' @param test A list of integer vectors, with the indices of the test set.
...
crossv_df_ <- function(n, test, train) {
  test <- test %||% purrr::rerun(length(train), NULL)
  train <- train %||% purrr::rerun(length(test), NULL)
